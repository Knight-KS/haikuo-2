  ...
  if (!Is(/m\.le\.com/) || !IsNot(/m.le.com\/search|so.le.com\/s|\.le\.com\/(ptv\/vplay\/|vplay_)/)) {
    ...
    if (HTML.getAttribute(PLUGIN_ATTR) !== PLUGIN_ID) {
      ...
      var collapseNum = isMobile ? 8 : 12,
        VIP_URLS = "";
      try {
        VIP_URLS = (window.VIP_URLS || VIP解析 || "").trim()
      } catch (n) {}
      ...
      if (Is(isVipUrlRegex) || Is(/oo\.movie&/)) Is(/=http/) && ($("title").html(PLUGIN_ID), _$((function() {
        ...
        $(".movie-reviews .show-all").after('<div class="_V_sign"><a href="https://gitee.com/ecruos/oo">豆瓣·净化 '.concat(OO_SIGN, "</a></div>")), $("section + .center").each((function(n, e) {
          $(e).remove()
  ...
  function insertVipSource(n) {
    ...
    if (!($(".oo-vip").length > 0 || t > 20)) {
      ...
      if (0 !== i.length) {
        ...
        })).join("\n")).concat(VIP_URLS.length > collapseNum ? '<span class="oo-vip-item oo-normal oo-collapse">➢</span>' : "", "\n  </div>\n  ").concat(isMobile ? '<div class="oo-vip-tip'.concat(isAllowSniff ? "" : " is-warning", '">配合浏览器的<strong>广告拦截</strong>和<strong>嗅探播放</strong>功能使用，体验更佳。').concat(isHiker ? "" : '<br>可自行拦截解析网站的图片广告，或订阅 <strong><a href="https://gitee.com/ecruos/oo/raw/master/scripts/oo.movie.adblock.txt" target="_blank">oo.movie adblock</a></strong> 拦截规则。', "</div>") : "", "\n</div>\n")), smartCollapse(".oo-vip-item");
        var r = !1;
        u();
        var a = 0,
